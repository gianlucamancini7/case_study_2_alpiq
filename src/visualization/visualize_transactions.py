...
def executed_transactions_bar_chart(pivoted_levels, plot=True, return_updated_transactions=False, new_data_type=False):
    """
    ...
    if return_updated_transactions:
        ...
        data_updated = pivoted_levels[pivoted_levels['match_binary_outcome'] == 1]['Transaction Time Hour'].value_counts(
        ).sort_index()
        ...
        ax.set_ylabel('closed transactions in time window', size=15)
        ...
        ax.set_ylabel('closed transactions in time window')
...
def executed_transactions_time_series(pivoted_levels, plot=True, output_series_='exec_price', rolling_time='1H', new_data_type=False):
    ...
    if plot:
        ...
        if output_series_ == 'exec_price_updated_transactions_comparison':
